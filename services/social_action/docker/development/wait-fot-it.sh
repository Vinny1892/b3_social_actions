...
echo "SELECT 'CREATE DATABASE social_action_development' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'credenciamento_teste')\gexec"| PGPASSWORD=$DB_PASSWORD psql -h  "$DB_HOST" -U  "$DB_USERNAME"
